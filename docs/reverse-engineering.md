...
How I recovered the protocol from NETGEAR's stock ReadyNAS firmware. This is
...
the `"Booting.."` string.) Nothing is broken. The panel just needs a driver.
...
See [`lcd-protocol.md`](lcd-protocol.md) for the resulting table.
...
So buttons = MSP430 `reg 0x04` bitmap over SMBus. I then confirmed the
bit→button mapping empirically by holding each button and watching `reg 0x04`
